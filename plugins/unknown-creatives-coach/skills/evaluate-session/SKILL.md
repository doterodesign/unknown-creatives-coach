...
1. Read `unknown-creatives-coach-knowledge/coach-feedback.md` if it exists — context on recent performance patterns. If not, check legacy `contra-coach-knowledge/coach-feedback.md` as read-only context.
2. Read `unknown-creatives-coach-knowledge/eval-health.md` if it exists — flags any evals under review. If not, check legacy `contra-coach-knowledge/eval-health.md` as read-only context.
